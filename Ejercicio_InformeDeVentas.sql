...
SELECT F.DNI, F.FECHA_VENTA, IFA.CANTIDAD FROM
facturas F INNER JOIN items_facturas IFA
...
SUM(IFA.CANTIDAD) AS CANTIDAD_VENDIDA, 
TC.VOLUMEN_DE_COMPRA/10 AS CANTIDAD_MAX
FROM facturas F INNER JOIN items_facturas IFA
ON F.NUMERO = IFA.NUMERO
...
A.CANTIDAD_VENDIDA - A.CANTIDAD_MAX AS DIFERENCIA FROM(
SELECT F.DNI,TC.NOMBRE, DATE_FORMAT(F.FECHA_VENTA, "%m/%Y") AS MES_AÑO, 
SUM(IFA.CANTIDAD) AS CANTIDAD_VENDIDA, 
TC.VOLUMEN_DE_COMPRA/10 AS CANTIDAD_MAX
FROM facturas F INNER JOIN items_facturas IFA
...
SELECT A.DNI, A.NOMBRE, A.MES_AÑO, A.CANTIDAD_VENDIDA, A.CANTIDAD_MAX, 
A.CANTIDAD_VENDIDA - A.CANTIDAD_MAX AS DIFERENCIA,
CASE
	WHEN (A.CANTIDAD_VENDIDA - A.CANTIDAD_MAX) <= 0 THEN 'Venta válida'
    ELSE 'Venta inválida'
END AS STATUS_VENTA
FROM(
SELECT F.DNI,TC.NOMBRE, DATE_FORMAT(F.FECHA_VENTA, "%m/%Y") AS MES_AÑO, 
SUM(IFA.CANTIDAD) AS CANTIDAD_VENDIDA, 
MAX(TC.VOLUMEN_DE_COMPRA)/10 AS CANTIDAD_MAX
FROM facturas F INNER JOIN items_facturas IFA
ON F.NUMERO = IFA.NUMERO
INNER JOIN tabla_de_clientes TC
ON TC.DNI = F.DNI
